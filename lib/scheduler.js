...
export async function runScheduler() {
  ...
  for (const debt of debts) {
    ...
    const hasOverdue = debt.installmentList.some(i => i.status !== 'paid' && diffDays(today, i.dueDate) < 0);

    const newStatus = allPaid ? 'paid' : hasOverdue ? 'overdue' : 'pending';
    if (debt.status !== newStatus) { debt.status = newStatus; changed = true; }

    if (allPaid && debt.status === 'paid') {
      await log(`✅ Dívida quitada: <strong>${debt.name}</strong> — ${debt.product}`, 'success');
    }
  ...
  return { ok: true, today, processed, total: debts.length };
}
